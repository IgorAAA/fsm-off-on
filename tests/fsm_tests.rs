...
fn scenario_from_task_description() {
    ...
    let mut fsm = Fsm::new(NonZeroUsize::new(3).unwrap(), CrossbeamSender(tx));
...
fn no_transition_when_signal_matches_current_state() {
    let (tx, rx) = crossbeam_channel::unbounded::<Signal>();
    let mut fsm = Fsm::new(NonZeroUsize::new(2).unwrap(), CrossbeamSender(tx));
...
fn transition_at_exact_threshold() {
    let (tx, rx) = crossbeam_channel::unbounded::<Signal>();
    let mut fsm = Fsm::new(NonZeroUsize::new(1).unwrap(), CrossbeamSender(tx));
